...
export const LoginDialog = ({ open, onOpenChange }: Props) => {
  ...
  const reset = () => { setMode("signin"); setEmail(""); setPassword(""); };
  const handleClose = (v: boolean) => { if (!busy) { onOpenChange(v); if (!v) reset(); } };

  const handleGoogle = async () => {
    setBusy(true);
    try {
      const result = await lovable.auth.signInWithOAuth("google", { redirect_uri: getAppRedirectUrl() });
      ...
      <DialogContent className="max-w-md p-0 overflow-hidden bg-card">
        <div className="p-8">
          ...
          <DialogDescription className="sr-only">
            Sign in or create an account to build your CV.
          </DialogDescription>
